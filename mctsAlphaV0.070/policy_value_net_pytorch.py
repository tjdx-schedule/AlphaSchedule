...
from config import scheConfig,obsConfig
...
class Net(nn.Module):
    ...
    def forward(self, state_input):
        ...
        x_act = F.log_softmax(self.dist0_linear(x_act))
...
class PolicyValueNet:
    ...
    def __init__(self, model_file=None, use_gpu=False, is_train = True):
        ...
        self.p, self.m = scheConfig.partNum,scheConfig.machNum
        self.n, self.f = obsConfig.stackFrameNum,obsConfig.ruleFeatureNum
        self.w, self.h = obsConfig.width, obsConfig.height
    ...
    def policy_value_sess(self, state_batch):
        current_state_first,current_state_third= state_batch #current_state_second,
        if self.use_gpu:
            log_act_probs, value = self.policy_value_net(
                    ...
                    # Variable(torch.from_numpy(current_state_second)).cuda().float(),
                     Variable(torch.from_numpy(current_state_third)).cuda().float()])
            ...
            log_act_probs, value = self.policy_value_net(
                    [Variable(torch.from_numpy(current_state_first)).float(),
                     ...
                     Variable(torch.from_numpy(current_state_third)).float()])
    ...
    def policy_value_fn(self, state, legal_positions):
        ...
        state_first, state_third = state #state_second,
        ...
        current_state_third = np.ascontiguousarray(np.expand_dims(state_third,0))

        act_probs,value = self.policy_value_sess([current_state_first,
                                                  # current_state_second,
                                                  current_state_third])
    ...
    def load_pretrained_weight(self, path, vec_norm, load_way='ori'):
        ...
        vec_norm.ret_rms.mean = weight_dict['ret_rms_mean']
        vec_norm.ret_rms.var = weight_dict['ret_rms_var']
