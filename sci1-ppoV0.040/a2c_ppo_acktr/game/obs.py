...
#        e = 1e-10
        ...
        wspt = -priority/(hours+e) #add
        wmdd = np.maximum(hours,deadline_cur)/(priority + e) #add
        atc = np.exp(delta/h) * priority/(hours+e)#ATC
        wco = np.maximum(1+delta/Kt,0) * priority/(hours+e)

        partFeatureMat = np.concatenate((hours, priority,
                        deadline_cur,deadline_cur_hours,
                        slack, \
                        wmdd, wspt, atc, wco),axis=1)       
