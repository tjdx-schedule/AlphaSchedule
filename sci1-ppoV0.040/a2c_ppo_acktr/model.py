...
class Policy(nn.Module):
    def __init__(self, obs_shape, action_space, v_obs_shape=None, base=None, base_kwargs=None):
        ...
        print('model class: ', self.base.__class__.__name__)
...
#        device = value.device
    ...
    def save_weight(self,path,ret_rms,name):
        name += '-weight.model'
        path += '/' + name
        ...
        if base_name == 'ResNet':
            ...
            weight_dict['com_base'] = self.base.state_dict()
            weight_dict['dist0_fc'] = self.dists[0].linear.state_dict()
    ...
    def load_pretrained_weight(self, path, load_way='ori'):
        ...
        self.base.res_tower.load_state_dict(weight_dict['com_base'])
        mean, var = weight_dict['ret_rms_mean'],weight_dict['ret_rms_var']
        return mean , var
        

class SimConvNet(nn.Module):
    def __init__(self, num_inputs_canvas, num_inputs_value, input_size):
        super().__init__()
        num_inputs = num_inputs_canvas
#        channels = [32, 64, 4]
#        fc_nums = [128]
        self.before_fc_channel = 4
        
        self.conv_flatten_size = input_size * self.before_fc_channel
        self.policy_output_size = 128#input_size * self.before_fc_channel
        ...
        self.conv1 = conv1x3(num_inputs, 32)
        self.bn1 = torch.nn.BatchNorm2d(32)
        self.conv2 = conv1x3(32, 64)
        self.bn2 = torch.nn.BatchNorm2d(64)
        
        self.conv3 = conv1x3(64, self.before_fc_channel)
        
        self.fc1 = nn.Linear(input_size*self.before_fc_channel, self.policy_output_size)
        self.fc_val = nn.Linear(self.policy_output_size, 1)
    ...
    def forward(self, visual_inputs, vec_inputs, rnn_hxs, masks):
        ...
        out = self.conv3(out)
        out = out.view(-1,self.conv_flatten_size)
...
def conv1x3(in_channels, out_channels):
    return torch.nn.Conv2d(
        in_channels, out_channels, kernel_size=[1,3], stride=1, padding=[0,1], bias=False
    )   
